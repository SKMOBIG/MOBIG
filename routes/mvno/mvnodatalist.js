module.exports = function(app, connectionPool) {
    ...
    app.get('/mvno/mvnodatalist', function(req, res, next) {
        ...
        console.log('/mvno/mvnodatalist/', user_id, page);

        // 해당유저의 요청테이블 조회(+유저명, 데이터명 Join, +결과테이블에 없으면 0 넘김)
        // rslt_data 수정 필요
        connectionPool.getConnection(function(err, connection) {
            var query = 'SELECT a.*, b.user_name, (SELECT category_grp_dtl_nm FROM mvno_category_grp_lst WHERE category_grp_dtl_id=a.category_grp_dtl_id1) category_grp_dtl_nm1, \
            ...
            connection.query(query, [user_id, cntPerPage, start], function(error, rows) {
                if (error) throw error;
                    ...
                    if (rows.length >= 0) {
                        ...
                        // 총개수 파악
                        var queryCnt = 'SELECT count(*) cnt FROM mvno_req_data WHERE user_id = 8;';
                        connection.query(queryCnt, user_id, function(error2, rowsCnt) {
                                if (error2) throw error2;
                                    ...
                                    // 한방에 렌더
                                    res.render('mvno/mvnodatalist', { req_list: rows, session: req.session, grpId: grpId, page: page, cntPerPage: cntPerPage, start: start, cnt: rowsCnt });
                                    connection.release();
                                    }
                        });
