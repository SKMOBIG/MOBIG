module.exports = function(app, connectionPool) {
    ...
    app.get('/mvno/mvnoresult/:id', function(req, res, next) {
        ...
        console.log('/mvno/mvnoresult/', req_id);
        ...
        connectionPool.getConnection(function(err, connection) {
            connection.query('select * from rslt_data where req_id=?;', req_id, function(error, rows) {
                if (error) {
                    ...
                    if (rows.length >= 0) {
                        res.render('mvno/mvnoresult', { rslt_list: rows, req_id: req_id, session: req.session });
                        connection.release();
                    }
                    else {
                        res.redirect('/');
    ...
    app.post('/mvno/complete', function(req, res){
        ...
        connectionPool.getConnection(function(err, connection) {
                if(err) {
                    ...
                    console.log('/mvno/complete/', req_id);
