...
module.exports = function(app, connectionPool) {
   ...
   app.post('/Happdaygetdata_KJY', function(req, res, next) {
    ...
    app.post('/happyday_update', function(req, res, next) {
        ...
        connectionPool.getConnection(function(err, connection) {
            connection.query('update happyday_master set happyday_name = ?, happyday_contents = ?, category_code=?, dday_dt = ?, happyday_dt = ?, req_point =?, num_participants =?, place_name = ?, img_url =?, point_rsn = ?, update_dtm = date_format(sysdate(), "%Y%m%d%H%i%s") where happyday_id = 205;'                       
